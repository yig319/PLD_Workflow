...
def preferred_channel_index(labels: list[str]) -> int:
    """Choose a sensible default channel for quick single-image preview."""
...
def _should_show_metric_overlay(channel_label: str, *, multiple_plots: bool) -> bool:
    ...
    In comprehensive mode the overlay is intentionally limited to the `Height`
    panel to keep multi-channel layouts less busy. In quick-view mode, the
    selected channel can still show its overlay text.
    """
